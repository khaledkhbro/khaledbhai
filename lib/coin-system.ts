...
export async function collectDailyCoins(userId: string): Promise<{
  ...
  const streakBonus = Math.min(
    Math.floor(baseReward * (rewardSettings.streakMultiplier - 1) * newStreak),
  ...
  return {
    ...
    bonusCoins: streakBonus,
    nextCollectionTime: nextCollection.toISOString(),
    message: `Congratulations! You earned ${totalCoins} coins${streakBonus > 0 ? ` (${streakBonus} streak bonus)` : ""}!`,
  }
}
